...
class BufferModel {
    ...
    addQuad(
        quad: map3D.LineQuad,
        topColour: THREE.Color = new THREE.Color(255, 255, 255),
        bottomColour: THREE.Color = new THREE.Color(255, 255, 255)
    ){
        ...
        function isOnTop(position: map3D.QuadVertexPosition): boolean {
            // Is the vertex on the top or bottom? True if on top.
            return (
                position === map3D.QuadVertexPosition.UpperLeft ||
                position === map3D.QuadVertexPosition.UpperRight);
        }
        ...
        const whiteColour = new THREE.Color(255, 255, 255);
        for(let vertexIterIndex = 0; vertexIterIndex < quadTriVertices.length; vertexIterIndex++){
            ...
            const lightGray = lightLevel / 255 + fakeContrast;
            ...
            if(topColour.equals(whiteColour) && bottomColour.equals(whiteColour)){
                // Vanilla lighting
                this.setBufferElement(BufferType.Color, [lightGray, lightGray, lightGray]);
            }else{
                this.setBufferElement(BufferType.Color, isOnTop(quadTriVertex) ? topColour.toArray() : bottomColour.toArray());
            }
    ...
    addTriangle(triangle: map3D.SectorTriangle, colour: THREE.Color = new THREE.Color(255, 255, 255)){
